...
const ignoredProjects = ['HB', 'TC'];

const searchForUnknownProjects = allDefinitions => {
  const jetbrainsCodes = definitions.map(x => x.jetbrainsCode);
  ...
  const missingCasks = allDefinitions
    .filter(product => {
      return (
        ...
        !ignoredProjects.includes(product.code) &&
    ...
    \t\tLast release: ${lastRelease.date}, ${lastDownload}`;
    });
...
Promise.all([getPrevious(), getCurrent(), allProducts()])
  .then(([previous, current, all]) => {
    searchForUnknownProjects(all);
