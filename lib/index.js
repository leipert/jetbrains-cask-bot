  ...
  .then(function({ updateAble, files, missingCasks }) {
    ...
    _.forEach(needUpdate, app => {
      ...
      const branch = `jcb_${app.name}_${app.jetbrains.version.replace(
        /\W/g,
        '_'
      )}_${app.jetbrains.sha256.substr(0,6)}`;
      ...
      try {
        ...
        let prTtitle = `Update ${app.name} to ${app.jetbrains.version}`;
        ...
        if(app.cask.version === app.jetbrains.version) {
            ...
            prTtitle = `Fix sha256 of ${app.name}@${app.jetbrains.version}`;
        }
    ...
    const missingAppCast = _
      .chain(updateAble)
      .reject('needUpdate')
      .filter('missingAppCast')
      .value();

    const outdatedAppCast = _
      .chain(updateAble)
      .reject('needUpdate')
