  ...
  return Promise.all([
    ...
    .then(mtimesArray => {
      ...
      const delKeys = _.chain(definitions)
        .map('caskName')
        .reject(caskName => _.get(oldTimes, caskName, true) === _.get(newTimes, caskName, false))
...
const checkAll = definitions => {
  ...
  return allProducts(definitions)
    ...
    .then(products => Promise.all(_.map(definitions, getStatus.bind(null, products))));
};
...
const getMissingCasks = definitions => {
  if (isCI) {
    console.log('Will not search for missing casks on ci');
    return;
  }
    ...
    cpPromise(`brew cask _stanza url --table | grep jetbrains`)
      ...
      .then(result => {
        return _.chain(result)
          ...
          .reject(_.isEmpty)
          .map(line => line.split('\t'))
          .reject(line => _.some(definitions, ({ caskName }) => _.endsWith(line[0], caskName)))
          .value();
      })
  )
    .then(missingCasks => {
