...
github.authenticate({
    type: 'token',
    token: process.env.JCB_GITHUB_API_TOKEN
});
...
const getOpenPRFiles = (prNumbers) => {
    return Promise
        .all(_.map(prNumbers, (prNum) => getPRFiles(prNum)));
};
...
const getCaskField = function(caskName, field) {
    return cpPromise(`brew cask _stanza ${field} ${caskName}`)
        .then((result) => {
            return result.stdout.replace(/\r?\n+/ig, '');
        });
};

const getCaskVersionOfApp = function({caskName}) {

    return Promise.props({
        version: getCaskField(caskName, 'version'),
        sha256: getCaskField(caskName, 'sha256'),
        url: getCaskField(caskName, 'url'),
    });
    ...
    return Promise
        ...
        .then(({name, cask, jetbrains}) => {
            return {
                ...
                cask,
                jetbrains,
                latest: _.isEqual(cask.version, jetbrains.version) && _.isEqual(cask.sha256, jetbrains.sha256)
            }
        });
...
Promise.props({
    ...
    .then(function({needupdate, files}) {
        ...
        _.forEach(needupdate, (app) => {
            ...
            const filePath = cpSync(`find "$(brew --repository)/Library/Taps/caskroom"  -name "${app.name}.rb"`).replace(/\r?\n+/ig, '');
            ...
            const result = cpSync(`bash ${patchScriptPath} ${dir} ${app.name} ${branch} ${app.jetbrains.version}`);
