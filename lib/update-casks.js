...
const getJBVersionOfApp = async (jetbrains, { jetbrainsCode, versionField, releaseChannel }) => {
...
const checkAll = async definitions => {
  ...
  return Promise.all(_.map(definitions, getStatus.bind(null, jetBrainsData)));
};
...
const main = async () => {
  ...
  log(`${_.size(latest)} of ${_.size(appsWithStatus)} casks are up to date (${latest.join(', ')})`);

  const updateAble = await _.filter(
    appsWithStatus,
  ...
  const casks = _.reject(needUpdate, app => {
